...
public void updateMatch(Match match) throws SQLException {
    Match existingMatch = matchRepository.findById(match.getMatchId())
        .orElseThrow(() -> new NoMatchesFoundException("Match not found"));

    Team firstTeam = teamRepository.findById(match.getFirstTeam().getTeamId()).orElseThrow();
    Team secondTeam = teamRepository.findById(match.getSecondTeam().getTeamId()).orElseThrow();
    Team winnerTeam = teamRepository.findById(match.getWinnerTeam().getTeamId()).orElseThrow();


    

    existingMatch.setFirstTeam(firstTeam);
    existingMatch.setSecondTeam(secondTeam);
    existingMatch.setWinnerTeam(winnerTeam);
